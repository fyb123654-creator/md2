...
public final class HouseCard implements ActionCard {
    private final String id;
    private final String name;
    private final int value;
    private static final int ADDED_RENT_VALUE = 3;
    ...
    public boolean execute(GameManager gameManager) {
        ...
        if (selectedColor == Color.RAILROAD || selectedColor == Color.UTILITY) {
            throw new IllegalStateException("House cannot be placed on railroad or utility sets: " + selectedColor);
        }
        if (!currentPlayer.isSetComplete(selectedColor)) {
            throw new IllegalStateException("House can only be placed on a complete property set: " + selectedColor);
        }
